...
public class LargeCreateStyled0100Visual implements BogeyVisual {
    ...
    private final TransformedInstance fullBlindWheel;
    ...
    public LargeCreateStyled0100Visual(VisualizationContext ctx, float partialTick, boolean inContraption) {
        ...
        // 创建全遮蔽轮
        fullBlindWheel = ctx.instancerProvider()
        ...
        pins = new TransformedInstance[5]; // -2, -1, 0, 1, 2
        for (int i = 0; i < 5; i++) {
    ...
    public void update(CompoundTag bogeyData, float wheelAngle, PoseStack poseStack) {
        ...
        // 更新纵向轴
        int[] sides = {-3, -2, -1, 1, 2, 3};
        for (int i = 0; i < 6; i++) {
            middleShafts[i].setTransform(poseStack)
                    .translate(-.5f, .25f, -1.3f + sides[i] * -1.6)
                    .center()
        ...
        // 更新全遮蔽轮
        fullBlindWheel.setTransform(poseStack)
                .scale(1 - 1 / 512f)
                .translate(0, 1, 0)
                .rotateXDegrees(wheelAngle)
                .translate(0, -1, 0)
